    ...
    Object.entries(tasksData).forEach(([taskId, task]) => {
          ...
          ? task.subtasks
              .map((subtask) =>
                typeof subtask === "string"
                  ? `<div class="subtask"><span>${subtask}</span></div>`
                  : `<div class="subtask"><input type="checkbox" ${
                      subtask.isChecked ? "checked" : ""
                    }><span>${subtask.subtask}</span></div>`
              )
              .join("")
          : "<p>No subtasks</p>";
      ...
      taskElement.innerHTML = `
            ...
            <div class="task-footer d-flex justify-content-between align-items-center">
                <div class="assigned-to">${(task.assignedTo || []).join(
                  ", "
                )}</div>
...
async function openModal(id) {
  ...
  document.getElementById("modalCard-delete-button").onclick = function () {
    deleteTaskOfModalCard(id);
  };
  document.getElementById("modal-card-edit-button").onclick = function () {
    changeToEditMode(id), putNextStatus(singleTask.status);
  };
    ...
    HTML += `
            <div class="d-flex align-items-center">
                <label class="custom-checkbox d-flex align-items-center" onclick="changeCheckbox('${taskId}', ${i})">
                    <input id="checkbox-subtask${i}" type="checkbox" ${
      singleSubtask.isChecked ? "checked" : ""
    }>
